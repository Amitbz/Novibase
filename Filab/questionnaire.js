/*
 ...
 Author: Rubén Ferreira Pérez */ $(function () {
    // RESULTS
    // -------------------------------------------------------
        ...
        self.send = function (data)
        {
            ...
            { // client validation OK
                ...
                if (answer)
                {
                    ...
                    var dict = {A24: " It is a disease in which the patient has split personality", A25: " It is an alteration in the mechanisms that regulate
mood and so the person who suffers it goes through periods of euphoria and depression", A26: " Bipolar disorder is an invention of doctors really not a
disease..", A27: " A breakup", A28: " In most cases it is a disease with a genetic component", A29: " Drugs", A30: " If genetic factors are determinants for
this disease", A31: " There are environmental factors that can influence whether or not develop the disease",A32: " We all have the same genes",A33: " It is a
rare disease and not much she knows",A34: " It is a flu-like illness that happens to almost everyone every year..",A35: " It is a chronic disorder that many
people suffer but with proper treatment can lead a normal life",};

                    var Dta = ko.toJS(qresult);
					var answers = [];
                    var id = String(Dta.KtekQuestionnaireResultEntity.coduser);
                    var itms = Dta.KtekQuestionnaireResultEntity.listsections[0].listitems; //Array of objects
                    for (i = 0; i < itms.length; i++) { answers[i] = itms[i].listcodanswer; }
                    $.ajax({
                        type: 'POST',
                        url: 'http://www.noviqr.eu/quanswer.php',
                        data: { Idml: id, a1: dict[""+answers[0]+""], a2: dict[""+answers[1]+""], a3: dict[""+answers[2]+""], a4: dict[""+answers[3]+""] },
                        success: function (data) {
                            if (data) {
                                alert("I KNEW IT!"+data);
                            }
                            else {
                                console.log("YAY");
                            }
                    ...
                    { // network alive
                        ...
                        { // web service result exists
                            ...
                            else
                            {
                                ...
                                if ( parseInt(result.data.Integer) != 999) {
                                    var url = "http://www.noviqr.eu/thank-you.php";
